"""
SafeFoam v3.0 — CNC Kopuk Kesici CAM
  Sekme 1: Model Import  (STL / OBJ yükle, 3D onizleme)
  Sekme 2: Profil Kesimi (airfoil — otomatik eksen, karbon tup delikleri)
  Sekme 3: Planform      (ust görünus)
"""
import tkinter as tk
from tkinter import ttk, filedialog, messagebox
import numpy as np
import matplotlib
matplotlib.use("TkAgg")
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
from matplotlib.figure import Figure
from mpl_toolkits.mplot3d import Axes3D  # noqa: F401

from core.mesh_import import (load_mesh, extract_profile, foam_bounds,
                               mesh_info, auto_axes)
from core.gcode import MachineConfig, generate_planform_gcode
from core.cut_path import insert_hole_traversal
from ui.sim_player import SimPlayer


# ── Kucuk yardimcilar ────────────────────────────────────────────────────────

def _lbl(parent, text, **kw):
    return ttk.Label(parent, text=text, **kw)

def _entry(parent, var, w=8):
    return ttk.Entry(parent, textvariable=var, width=w)

def _row(parent, label, default, row, unit="", w=8):
    _lbl(parent, label).grid(row=row, column=0, sticky="w", pady=2, padx=(0, 4))
    var = tk.StringVar(value=str(default))
    _entry(parent, var, w).grid(row=row, column=1, pady=2)
    if unit:
        _lbl(parent, unit, foreground="#777").grid(row=row, column=2, sticky="w")
    return var

def _section(parent, title):
    f = ttk.LabelFrame(parent, text=title, padding=6)
    f.pack(fill="x", padx=6, pady=3)
    return f


# ════════════════════════════════════════════════════════════════════════════
#  TAB 1 — Model Import
# ════════════════════════════════════════════════════════════════════════════

class ImportTab(ttk.Frame):

    def __init__(self, parent, app):
        super().__init__(parent, padding=4)
        self._app  = app
        self._mesh = None
        self._build()

    def _build(self):
        left = ttk.Frame(self, width=220)
        left.pack(side="left", fill="y", padx=(0, 6))
        left.pack_propagate(False)

        # Dosya sec
        ff = _section(left, "3D Model")
        ttk.Button(ff, text="Dosya Ac...  (STL / OBJ)",
                   command=self._open_file).pack(fill="x", pady=2)
        self._file_lbl = ttk.Label(ff, text="Dosya secilmedi",
                                   foreground="#888", wraplength=190)
        self._file_lbl.pack(fill="x", pady=2)

        # Model boyut + eksen bilgisi
        self._info_frm = _section(left, "Model Bilgisi")
        self._info_lbl = ttk.Label(self._info_frm,
                                   text="—", foreground="#aaa",
                                   justify="left")
        self._info_lbl.pack(anchor="w")

        # Olcek
        sf = _section(left, "Olcek")
        self._v_scale = _row(sf, "Olcek", "1.0", 0, "x")
        ttk.Button(sf, text="Uygula",
                   command=self._apply_scale).grid(
                       row=1, column=0, columnspan=3, sticky="ew", pady=4)

        ttk.Separator(left).pack(fill="x", pady=6)
        ttk.Button(left, text="Profil Kesimi (Pass 1) ->",
                   command=lambda: self._app.goto_pass(1)).pack(fill="x", pady=2)
        ttk.Button(left, text="Planform Kesimi (Pass 2) ->",
                   command=lambda: self._app.goto_pass(2)).pack(fill="x", pady=2)

        # 3D onizleme
        right = ttk.Frame(self)
        right.pack(side="left", fill="both", expand=True)

        fig = Figure(figsize=(6, 5), dpi=96, facecolor="#0f0f1e")
        self._fig3d = fig
        self._ax3d  = fig.add_subplot(111, projection="3d")
        self._canvas3d = FigureCanvasTkAgg(fig, master=right)
        self._canvas3d.get_tk_widget().pack(fill="both", expand=True)
        self._draw_empty()

    def _draw_empty(self):
        ax = self._ax3d
        ax.cla()
        ax.set_facecolor("#0f0f1e")
        ax.set_title("3D Model Onizleme", color="white", fontsize=9)
        for p in [ax.xaxis.pane, ax.yaxis.pane, ax.zaxis.pane]:
            p.set_facecolor("#12122a"); p.set_alpha(0.5)
        self._canvas3d.draw_idle()

    def _open_file(self):
        path = filedialog.askopenfilename(
            title="3D Model Ac",
            filetypes=[("3D Dosyalar", "*.stl *.obj *.ply *.3mf"),
                       ("Tum dosyalar", "*.*")])
        if not path:
            return
        try:
            mesh = load_mesh(path)
        except Exception as e:
            messagebox.showerror("Yukleme Hatasi", str(e))
            return

        self._mesh = mesh
        self._app.set_mesh(mesh)
        self._file_lbl.config(text=path.split("/")[-1].split("\\")[-1],
                              foreground="#4fc3f7")
        self._update_info()
        self._draw_mesh()

    def _apply_scale(self):
        if self._mesh is None:
            return
        try:
            s = float(self._v_scale.get())
        except ValueError:
            return
        import trimesh
        m = self._mesh.copy()
        m.apply_scale(s)
        m.apply_translation(-m.centroid)
        self._mesh = m
        self._app.set_mesh(m)
        self._update_info()
        self._draw_mesh()

    def _update_info(self):
        if self._mesh is None:
            return
        i = mesh_info(self._mesh)
        airfoil_ax, planform_ax = auto_axes(self._mesh)
        ax_label = {
            'X': "X (sol-sag)", 'Y': "Y (on-arka)", 'Z': "Z (yukari)"
        }
        self._info_lbl.config(
            text=(f"X: {i['x']:.1f} mm\n"
                  f"Y: {i['y']:.1f} mm\n"
                  f"Z: {i['z']:.1f} mm\n"
                  f"Vertices: {i['vertices']:,}\n"
                  f"Faces:    {i['faces']:,}\n"
                  f"─────────────\n"
                  f"Aciklik ekseni: {ax_label[airfoil_ax]}\n"
                  f"Profil     (P1): {airfoil_ax} boyunca\n"
                  f"Planform   (P2): {planform_ax} boyunca"))

    def _draw_mesh(self):
        if self._mesh is None:
            return

        self._fig3d.clear()
        ax = self._fig3d.add_subplot(111, projection="3d")
        self._ax3d = ax
        ax.set_facecolor("#0f0f1e")

        m     = self._mesh
        verts = m.vertices
        edges = m.edges_unique

        step = max(1, len(edges) // 4000)
        ex, ey, ez = [], [], []
        for e in edges[::step]:
            ex += [verts[e[0], 0], verts[e[1], 0], None]
            ey += [verts[e[0], 1], verts[e[1], 1], None]
            ez += [verts[e[0], 2], verts[e[1], 2], None]

        ax.plot(ex, ey, ez, color="#4fc3f7", lw=0.4, alpha=0.6)

        b   = m.bounds
        pad = (b[1] - b[0]) * 0.05
        ax.set_xlim(b[0][0] - pad[0], b[1][0] + pad[0])
        ax.set_ylim(b[0][1] - pad[1], b[1][1] + pad[1])
        ax.set_zlim(b[0][2] - pad[2], b[1][2] + pad[2])
        ax.set_xlabel("X (mm)", color="#666", fontsize=7)
        ax.set_ylabel("Y (mm)", color="#666", fontsize=7)
        ax.set_zlabel("Z (mm)", color="#666", fontsize=7)
        ax.tick_params(colors="#555", labelsize=6)
        ax.set_title("3D Model Onizleme", color="white", fontsize=9)
        for p in [ax.xaxis.pane, ax.yaxis.pane, ax.zaxis.pane]:
            p.set_facecolor("#0a0a1a"); p.set_alpha(0.6)
        ax.view_init(elev=20, azim=-50)
        self._canvas3d.draw()


# ════════════════════════════════════════════════════════════════════════════
#  TAB 2 & 3 — Pass (Profil / Planform)
# ════════════════════════════════════════════════════════════════════════════

class PassTab(ttk.Frame):

    VIEW_LABELS = {
        'X': "X ekseni boyunca (YZ duzlemi)",
        'Y': "Y ekseni boyunca (XZ duzlemi)",
        'Z': "Z ekseni boyunca (XY duzlemi)",
    }

    def __init__(self, parent, app, pass_num: int):
        super().__init__(parent, padding=4)
        self._app      = app
        self._pass_num = pass_num
        self._profile  = (None, None)
        self._bounds   = None
        self._hole_rows = []   # [(var_en, var_hx, var_hy, var_hr), ...]
        self._build()

    # ── UI ──────────────────────────────────────────────────────────────────

    def _build(self):
        left = ttk.Frame(self, width=230)
        left.pack(side="left", fill="y", padx=(0, 6))
        left.pack_propagate(False)

        # Kesim yonu
        df = _section(left, "Projeksiyon Ekseni")
        self._v_axis = tk.StringVar(value='Y')
        for axis, lbl in self.VIEW_LABELS.items():
            ttk.Radiobutton(df, text=lbl, value=axis,
                            variable=self._v_axis,
                            command=self._on_axis_change).pack(anchor="w", pady=1)

        # Makine
        mf = _section(left, "Makine")
        self._v_feed   = _row(mf, "Kesim hizi", 300, 0, "mm/min")
        self._v_plunge = _row(mf, "Yaklasma",   100, 1, "mm/min")
        self._v_leadin = _row(mf, "Lead-in",     15, 2, "mm")

        # Eksen eslestirme
        af = _section(left, f"Eksen Eslestirme (Pass {self._pass_num})")
        self._v_ax1h = _row(af, "Kule 1 yatay", "X", 0, w=4)
        self._v_ax1v = _row(af, "Kule 1 dikey", "Y", 1, w=4)
        self._v_ax2h = _row(af, "Kule 2 yatay", "A", 2, w=4)
        self._v_ax2v = _row(af, "Kule 2 dikey", "B", 3, w=4)

        # Karbon tup delikleri — sadece Pass 1
        if self._pass_num == 1:
            self._build_holes_section(left)

        ttk.Separator(left).pack(fill="x", pady=4)
        ttk.Button(left, text="Profili Yenile",
                   command=self._refresh).pack(fill="x", pady=2)
        ttk.Button(left, text="G-Code Uret & Kaydet",
                   command=self._save_gcode).pack(fill="x", pady=2)

        if self._pass_num == 1:
            ttk.Button(left, text="Planform (Pass 2) ->",
                       command=lambda: self._app.goto_pass(2)).pack(
                           fill="x", pady=(6, 2))

        # SimPlayer
        self._sim = SimPlayer(
            self,
            title=("Pass 1 — Profil Kesimi" if self._pass_num == 1
                   else "Pass 2 — Planform Kesimi"))
        self._sim.pack(side="left", fill="both", expand=True)

    def _build_holes_section(self, parent):
        hf = _section(parent, "Karbon Tup Delikleri")

        # Aktif toggle
        self._v_holes_en = tk.BooleanVar(value=False)
        ttk.Checkbutton(hf, text="Delik gecisi aktif",
                        variable=self._v_holes_en).grid(
                            row=0, column=0, columnspan=5, sticky="w")

        # Basliklar
        for col, txt in enumerate(["", "HX", "HY", "R", "mm"]):
            ttk.Label(hf, text=txt, foreground="#777",
                      font=("Segoe UI", 7)).grid(
                          row=1, column=col, padx=1)

        # 3 delik satiri (varsayilan degerler tipik RC kanadi icin)
        defaults = [
            (True,  -50.0, -8.0, 6.0),
            (True,    0.0, -8.0, 6.0),
            (False,  45.0, -6.0, 5.0),
        ]
        self._hole_rows = []
        for i, (en, hx, hy, hr) in enumerate(defaults):
            row = i + 2
            v_en = tk.BooleanVar(value=en)
            v_hx = tk.StringVar(value=str(hx))
            v_hy = tk.StringVar(value=str(hy))
            v_hr = tk.StringVar(value=str(hr))

            ttk.Checkbutton(hf, variable=v_en).grid(row=row, column=0, padx=1)
            ttk.Entry(hf, textvariable=v_hx, width=5).grid(row=row, column=1, padx=1)
            ttk.Entry(hf, textvariable=v_hy, width=5).grid(row=row, column=2, padx=1)
            ttk.Entry(hf, textvariable=v_hr, width=4).grid(row=row, column=3, padx=1)
            ttk.Label(hf, text=f"D{i+1}", foreground="#aaa",
                      font=("Segoe UI", 7)).grid(row=row, column=4, padx=2)
            self._hole_rows.append((v_en, v_hx, v_hy, v_hr))

        # Uygula butonu
        ttk.Button(hf, text="Uygula",
                   command=self._refresh).grid(
                       row=len(defaults) + 2, column=0, columnspan=5,
                       sticky="ew", pady=(4, 0))

    def _get_holes(self):
        """Aktif delik listesini dondur: [(hx, hy, hr), ...]"""
        if self._pass_num != 1:
            return []
        if not self._v_holes_en.get():
            return []
        holes = []
        for v_en, v_hx, v_hy, v_hr in self._hole_rows:
            if not v_en.get():
                continue
            try:
                holes.append((float(v_hx.get()),
                               float(v_hy.get()),
                               float(v_hr.get())))
            except ValueError:
                pass
        return holes

    # ── Eksen degisince ──────────────────────────────────────────────────────

    def _on_axis_change(self):
        """Eksen radio butonu degisince profili hemen yenile."""
        self._profile = (None, None)
        self._refresh()

    def auto_set_axis(self, airfoil_ax: str, planform_ax: str):
        """Mesh yuklenince otomatik eksen ayarla."""
        if self._pass_num == 1:
            self._v_axis.set(airfoil_ax)
        else:
            self._v_axis.set(planform_ax)

    # ── Profil ──────────────────────────────────────────────────────────────

    def refresh_from_mesh(self, mesh):
        if mesh is None:
            self._sim.clear()
            return
        axis = self._v_axis.get()
        try:
            px, py = extract_profile(mesh, view_axis=axis)
            if px is None:
                messagebox.showwarning("Profil", "Bu eksende profil cikarilamadi.")
                return
            bd = foam_bounds(mesh, view_axis=axis)
            self._profile = (px, py)
            self._bounds  = bd
        except Exception as e:
            messagebox.showerror("Profil Hatasi", str(e))
            return

        holes = self._get_holes()
        if holes:
            try:
                px_mod, py_mod = insert_hole_traversal(px, py, holes)
            except Exception:
                px_mod, py_mod = px, py
        else:
            px_mod, py_mod = px, py

        self._sim.set_data(px_mod, py_mod, bd, holes=holes)

    def on_show(self):
        """Sekme gozukuncе cagrılır."""
        mesh = self._app.mesh
        if mesh is not None and self._profile[0] is None:
            self.refresh_from_mesh(mesh)
        elif self._profile[0] is not None:
            holes = self._get_holes()
            px, py = self._profile
            if holes:
                try:
                    px, py = insert_hole_traversal(px, py, holes)
                except Exception:
                    pass
            self._sim.set_data(px, py, self._bounds, holes=holes)

    def _refresh(self):
        self._profile = (None, None)
        self.refresh_from_mesh(self._app.mesh)

    # ── G-Code ──────────────────────────────────────────────────────────────

    def _save_gcode(self):
        px, py = self._profile
        if px is None:
            messagebox.showwarning("G-Code", "Once bir model yukleyin.")
            return

        # Delik gecisi varsa modifiye profili kullan
        holes = self._get_holes()
        if holes:
            try:
                px, py = insert_hole_traversal(px, py, holes)
            except Exception:
                pass

        mc = MachineConfig(
            ax1_h=self._v_ax1h.get(), ax1_v=self._v_ax1v.get(),
            ax2_h=self._v_ax2h.get(), ax2_v=self._v_ax2v.get(),
            feed_rate=float(self._v_feed.get()),
            plunge_rate=float(self._v_plunge.get()),
            lead_in=float(self._v_leadin.get()),
        )
        gcode = generate_planform_gcode((px, py), mc)

        path = filedialog.asksaveasfilename(
            defaultextension=".nc",
            filetypes=[("G-Code", "*.nc *.gcode *.txt"), ("Tumu", "*.*")],
            initialfile=f"pass{self._pass_num}.nc",
            title="G-Code Kaydet",
        )
        if not path:
            return
        with open(path, "w", encoding="utf-8") as f:
            f.write(gcode)
        messagebox.showinfo("Kaydedildi",
                            f"Pass {self._pass_num} G-Code kaydedildi:\n{path}")


# ════════════════════════════════════════════════════════════════════════════
#  Ana Uygulama
# ════════════════════════════════════════════════════════════════════════════

class FoamCutterApp(tk.Tk):

    def __init__(self):
        super().__init__()
        self.title("SafeFoam v3 — CNC Kopuk Kesici")
        self.minsize(1080, 660)
        self.resizable(True, True)

        self.mesh          = None
        self.airfoil_axis  = 'Y'
        self.planform_axis = 'Z'

        self._build()

    def _build(self):
        bar = tk.Frame(self, bg="#12122a", height=36)
        bar.pack(fill="x")
        bar.pack_propagate(False)
        tk.Label(bar, text="  SafeFoam  CNC Kopuk Kesici",
                 bg="#12122a", fg="#4fc3f7",
                 font=("Segoe UI", 11, "bold")).pack(side="left", padx=8)
        tk.Label(bar, text="v3.0",
                 bg="#12122a", fg="#FF6B35",
                 font=("Segoe UI", 9, "bold")).pack(side="left")
        tk.Label(bar,
                 text="  Profil ekseni otomatik algilama | Karbon tup delik gecisi",
                 bg="#12122a", fg="#555",
                 font=("Segoe UI", 8)).pack(side="left", padx=8)

        nb = ttk.Notebook(self)
        nb.pack(fill="both", expand=True, padx=6, pady=6)
        self._nb = nb

        self.tab_import = ImportTab(nb, self)
        self.tab_pass1  = PassTab(nb, self, pass_num=1)
        self.tab_pass2  = PassTab(nb, self, pass_num=2)

        nb.add(self.tab_import, text="  1  Model Import  ")
        nb.add(self.tab_pass1,  text="  2  Profil Kesimi  ")
        nb.add(self.tab_pass2,  text="  3  Planform  ")

        nb.bind("<<NotebookTabChanged>>", self._on_tab_change)

    def set_mesh(self, mesh):
        self.mesh = mesh
        airfoil_ax, planform_ax = auto_axes(mesh)
        self.airfoil_axis  = airfoil_ax
        self.planform_axis = planform_ax
        # Cache'i sifirla + ekseni otomatik ayarla
        self.tab_pass1._profile = (None, None)
        self.tab_pass2._profile = (None, None)
        self.tab_pass1.auto_set_axis(airfoil_ax, planform_ax)
        self.tab_pass2.auto_set_axis(airfoil_ax, planform_ax)

    def goto_pass(self, num: int):
        self._nb.select(num)

    def _on_tab_change(self, event):
        idx = self._nb.index("current")
        if idx == 1:
            self.tab_pass1.on_show()
        elif idx == 2:
            self.tab_pass2.on_show()
